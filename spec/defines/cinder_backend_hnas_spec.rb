...
describe 'cinder::backend::hnas' do
 ...
 shared_examples_for 'hnas volume driver' do
  let :params do
    ...
    it 'configures hnas volume driver' do
      is_expected.to contain_cinder_config('hnas/volume_driver').with_value('cinder.volume.drivers.hds.iscsi.HDSISCSIDriver')
      params.each_pair do |config,value|
        is_expected.to contain_cinder_config('hnas/#{config}').with_value( value )
      end
    end
end
