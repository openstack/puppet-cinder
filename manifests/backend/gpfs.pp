...
define cinder::backend::gpfs (
  $gpfs_mount_point_base,
  $gpfs_images_dir        = undef,
  $gpfs_images_share_mode = undef,
  ...
  if $gpfs_images_share_mode and ! ($gpfs_images_share_mode in ['copy', 'copy_on_write']) {
    fail('gpfs_images_share_mode only support `copy` or `copy_on_write`')
  }
  if $gpfs_images_share_mode and ($gpfs_images_share_mode in ['copy', 'copy_on_write']) and ! $gpfs_images_dir {
